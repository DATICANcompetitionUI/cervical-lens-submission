...
import { Avatar } from "@cervical-lens/ui";
...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.5 3v18M14.5 3v18M4 8h4m8 0h4M4 16h4m8 0h4" />
      </svg>
    ),
  },
  {
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-40 bg-midnight/30 backdrop-blur-xs lg:hidden transition-opacity duration-300"
          onClick={onClose}
        />
      )}
      ...
      {/* Sidebar Panel */}
      <aside
        className={`fixed left-0 top-0 h-screen w-64 bg-parchment border-r border-sand flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
        <div className="p-6 border-b border-sand flex items-center justify-between">
          <Link href="/dashboard" className="flex items-center gap-3" onClick={onClose}>
            ...
            <div>
              <h1 className="text-base font-bold text-on-surface font-sans">
                CervicalLens
              </h1>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onClose}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-colors font-sans ${
                  isActive
        ...
        <div className="p-4 border-t border-sand">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-on-surface truncate font-sans">
                Pathologist
              </p>
              <p className="text-xs text-steel truncate font-sans">
                View profile
              </p>
            </div>
          </div>
